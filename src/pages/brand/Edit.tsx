...
export default function EditBrand() {
  ...
  const submitEdit = async (e: any) => {
    ...
    formData.append("id_brand", id);
    formData.append("name_brand", nama);
    formData.append("photo", image);

    try {
      const result = await myAxios.patch("/api/brands/edit-brand", formData, {
  ...
  return (
    <div className="p-8 w-full flex flex-col gap-y-5 items-center">
      ...
      <form className="w-1/2 flex flex-col items-center gap-y-5">
        ...
        <button onClick={submitEdit} className="px-8 py-1 rounded bg-sky-600">
          Submit
        </button>
      </form>
    </div>
